...
namespace FRSML {
	...
	namespace nmmintrin {
		...
		__m128 _Log(__m128 _para) {
			...
			__m128 one = *(__m128*)_D_1;
			__m128 x = _para;

			__m128 invaild_mask = _mm_cmple_ps(x, _mm_setzero_ps());
			x = _mm_max_ps(x, *(__m128*)0x00800000);
			
			emm0 = _mm_srli_epi32(_mm_castps_si128(x), 23);

			x = _mm_and_ps(x, invaild_mask);
			x = _mm_or_ps(x, *(__m128*)_D_half);
			...
			__m128 mask = _mm_cmplt_ps(x, *(__m128*)_D_cephes_SQRTHF);
			...
			__m128 y = *(__m128*)_D_cephes_log_p0;
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p0);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p1);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p2);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p3);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p4);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p5);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p6);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p7);
			y = _mm_mul_ps(y, x);
			y = _mm_add_ps(y, *(__m128*)_D_cephes_log_p8);
			y = _mm_mul_ps(y, _mm_mul_ps(x, z));

			tmp = _mm_mul_ps(e, *(__m128*)_D_cephes_log_q1);
			y = _mm_add_ps(y, tmp);

			//Precision
			tmp = _mm_mul_ps(z, *(__m128*)_D_half);
			y = _mm_sub_ps(y, tmp);

			tmp = _mm_mul_ps(e, *(__m128*)_D_cephes_log_q2);
			x = _mm_add_ps(x, _mm_add_ps(y,tmp));
			x = _mm_or_ps(x, invaild_mask);

			return x;
	
		}

		__m128d _Exp(__m128d _para) {
			__m128d tmp = _mm_setzero_pd();
			__m128d x = _para;
			__m128d fx; __m128i emm0;
			...
			fx = _mm_add_pd(fx, *(__m128d*)_D_half);

			emm0 = _mm_cvttpd_epi32(fx);
			tmp = _mm_cvtepi32_pd(emm0);

			__m128d flag = _mm_cmpgt_pd(tmp, fx);
			...
			x = _mm_sub_pd(x, _mm_add_pd(tmp,tmp2));
